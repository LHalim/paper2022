...
class wedgeTACS(TacsSteadyInterface):
    def __init__(self, comm, tacs_comm, model, n_tacs_procs):
        ...
        if comm.Get_rank() < n_tacs_procs:
            ...
            cte = 24.0e-6
            kappa = 230.0
            prop = constitutive.MaterialProperties(rho=rho, specific_heat=specific_heat, E=E, nu=nu, ys=ys, cte=cte, kappa=kappa)
            con = constitutive.IsoShellConstitutive(prop, t=1.0, tNum=0)
            transform = None
            quad_basis = elements.Quad4ThermalShell(transform, con)
            #model_plate = elements.LinearThermoelasticity3D(con)
            ...
            element_plate = elements.Quad4ThermalShell(transform, con)
            varsPerNode = 7 #model_plate.getVarsPerNode()

            # Load in the mesh
            mesh = TACS.MeshLoader(tacs_comm)
            mesh.scanBDFFile('nastran_CAPS3_coarse.bdf')
